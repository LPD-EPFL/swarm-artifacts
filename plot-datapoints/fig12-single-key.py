...
for workload in ["A"]:
  for clients in [16]:
    ...
    for subplot, op in zip(subplots, ops):
            for i, s in enumerate(schemes):
                    path = os.path.join('logs',
                        f'fig12-single-key/workload-{workload}/{clients}clients/{s}/client1.txt',
                    )
                    data = parse(path)[op]
                    xs = []
                    for ptile in ptiles:
    ...
    plt.savefig(f'output-plots/fig12-single-key.pdf', format='pdf', bbox_inches = 'tight', pad_inches=0.01)
