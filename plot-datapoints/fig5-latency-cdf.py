...
for workload in ["B"]:
    ...
    for subplot, op in zip(subplots, ops):
            for i, s in enumerate(schemes):
                    path = os.path.join('logs',
                        f'fig5-latency-cdf/workload-{workload}/{s}/client1.txt',
                    )
                    data = parse(path)[op]
                    xs = []
                    for ptile in ptiles:
            ...
            subplot.set_xlim([0, 5.2 if op == 'GET' else 13])
    ...
    plt.savefig(f'output-plots/fig5-latency-cdf.pdf', format='pdf', bbox_inches = 'tight', pad_inches=0.01)
