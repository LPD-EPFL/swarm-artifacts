...
for workload in ["B"]:
    ...
    for subplot, op in zip(subplots, ops):
            for i, s in enumerate(schemes):
                    path = os.path.join('logs',
                        f'newfig-limited-cache/5MiB/workload-{workload}/{s}/client1.txt',
                    )
                    data = parse(path)[op]
                    xs = []
                    for ptile in ptiles:
    ...
    plt.savefig(f'output-plots/newfig-limited-cache.pdf', format='pdf', bbox_inches = 'tight', pad_inches=0.01)
