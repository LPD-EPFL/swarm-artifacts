...
for op in ['GET','UPDATE']:
    ...
    for x,y in zip(xs, ys):
        ...
        if x > 11000:
            break
        print(f' at {round(x,0)}ms -> {op}s latency: {round(y,2)}μs')
...
print(f'Note: datapoints are grouped by batch of grain={grain} for the output to be human-readable. The graph uses different grains (grain=20 for the zoomed out version and grain=1 for the zoomed in version).')